...
class UsdqnOneDoFSimulatorDiscreteActionsSemi(object):
    ...
    def __init__(self, is_training=True):
        ...
        self.discrete_actions = np.arange(-155, 155, 1)
...
class UsdqnOneDoFSimulatorTwoActionsSl(object):
    ...
    def load_dataset(self):
        if self.is_training:
            ...
            self.raw_images = np.load('../data/1dof/usdqn-images-testing.npy')
            self.raw_labels = np.load('../data/1dof/usdqn-labels-testing.npy')
            self.data_discrete = np.arange(-3.05, 3.05, 0.02)
            self.goal_positions = np.array([
                self.min_action + 90*(1/RAD2DEG) - 2*(1/RAD2DEG),
                self.min_action + 270*(1/RAD2DEG) + 2*(1/RAD2DEG)
            ])
        ...
        print("Num goals before reduction:", goals)

        disc_indx = nearest_neighbors(self.data_discrete, self.raw_labels)

        self.labels = self.labels[disc_indx]
        goals = np.argwhere(self.labels == 1)
        print("Num goals after reduction:", goals)
        self.images = self.raw_images[disc_indx, 2:-2, 2:-2]
    ...
    def reset(self, np_random):
        rnd_angle = np_random.randint(low=-90, high=90)
    ...
    def get_angle(self):
        if self.current_indx is None:
            raise 
        return self.current_indx * 0.02

    def get_goal_angle(self):
        goals = np.argwhere(self.labels == 1) * 0.02
        #return goals[1][0] * 0.01, goals[1][0] * 0.01,
        return goals
        # if goals[0][0] < int(len(self.labels) / 4):
        #     return goals[0][0] * 0.02
        # else:
        #     return goals[1][0] * 0.02
...
class UsdqnBaseStateSpace(object):
    ...
    def _load_dataset(self):
        ...
        self.wheel = digitize_indexes(self.labels, bins)

    def reset(self):
        # Reset state space at random angle
        rnd_ind = np.random.choice(self.wheel, 1)
        self.wheel = np.roll(self.wheel, rnd_angle, axis=0)





class UsdqnOneDoFSimulatorTwoActionsSemi(object):
    ...
    def _render(self, mode='human', close=False):
        ...
        if mode == 'rgb_array':
            ...
            if self.viewer is None:
                ...
                self.needle_transform.set_rotation(self.usdqn_sim.get_goal_angle()[0][0])
                needle_plane.add_attr(self.needle_transform)
                self.viewer.add_geom(needle_plane)

                needle_plane1 = rendering.Line((0, -1), (0, 1))
                needle_plane1.set_color(68/255,108/255,179/255)
                needle_plane1.linewidth = 4
                needle_plane1.add_attr(rendering.LineWidth(4))
                self.needle_transform1 = rendering.Transform()
                self.needle_transform1.set_rotation(self.usdqn_sim.get_goal_angle()[1][0])
                needle_plane1.add_attr(self.needle_transform1)
                self.viewer.add_geom(needle_plane1)
