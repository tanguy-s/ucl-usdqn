...
def get_graph(state):
        ...
        with tf.variable_scope('conv1'):
            W_conv1 = weights([3, 3, 1, 16], '%sconv1' % 1)
            b_conv1 = biases([16], '%sconv1' % 1)
            out_conv1 = tf.nn.relu(
                            tf.nn.conv2d(state, W_conv1, 
                                strides=[1, 1, 1, 1], padding='SAME') + b_conv1)

        with tf.variable_scope('conv2'):
            W_conv2 = weights([3, 3, 16, 32], '%sconv2' % 1)
            b_conv2 = biases([32], '%sconv2' % 1)
            out_conv2 = tf.nn.relu(
                            tf.nn.conv2d(out_conv1, W_conv2, 
                                strides=[1, 1, 1, 1], padding='SAME') + b_conv2)

        with tf.variable_scope('conv3'):
            W_conv3 = weights([3, 3, 32, 64], '%sconv2' % 1)
            b_conv3 = biases([64], '%sconv2' % 1)
            out_conv3 = tf.nn.relu(
                            tf.nn.conv2d(out_conv2, W_conv3, 
                                strides=[1, 2, 2, 1], padding='SAME') + b_conv3)
            # Flatten conv 1 output
        ...
        with tf.variable_scope('fully_connected'):
            W_fc = weights([conv3_dim, 1024], '%sfc' % 1)
            b_fc = biases([1024], '%sfc' % 1)
            out_fc = tf.nn.relu(tf.matmul(out_conv3_flat, W_fc) + b_fc)

        with tf.variable_scope('linear'):
            W_lin = weights([1024, DIM_OUT], '%slin' % 1)
            b_lin = biases([DIM_OUT], '%slin' % 1)
